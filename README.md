...
The digital/debug header `J1` uses SparkFun's staggered `1x16_Locking` friction-fit footprint, vendored at [`footprints/SparkFun-Connector.pretty/1x16_Locking.kicad_mod`](footprints/SparkFun-Connector.pretty/1x16_Locking.kicad_mod) from `sparkfun/SparkFun-KiCad-Libraries` commit `14865e2`. It keeps the 2.54 mm header pitch but alternates pad positions by 0.127 mm so a standard breakaway male header can lock into the board without solder during bench use. The SparkFun library is licensed CC BY 4.0; attribution: SparkFun Electronics.
...
The voltage screw-terminal footprints use KiCad's 1x02 3.50 mm horizontal Phoenix STEP model as the closest installed mechanical equivalent for the 4Ucon footprint, with model-only offset `(-1.75, -0.05, 0)` to align the substitute body to the 4Ucon footprint outline.
